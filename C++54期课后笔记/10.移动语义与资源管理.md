...
> //为什么会出问题：
> 
> shared_ptr 采用引用计数来管理资源，当最后一个 shared_ptr 销毁时，会调用删除器释放该资源。
> 
> 这里 sp 和 sp2 分别独立构造，它们各自构造时会将 pt 的引用计数初始化为 1。
> 
> 这意味着两个智能指针各自认为自己拥有 pt 的所有权，最终在各自析构时都将调用 delete pt。
> 
> 结果：当其中一个 shared_ptr 释放资源后，另一个 shared_ptr 再释放时，就会出现双重删除，导致未定义行为（如程序崩溃）。
>     //正确用法：
> 让裸指针只传给一个 shared_ptr，如果需要共享，应通过拷贝构造
>     （即 shared_ptr<Point> sp2 = sp;），这样它们共享同一个引用计数，而不是各自管理同一个裸指针。
> 
>     void test3(){
>  //使用不同的智能指针托管同一片堆空间
...
> }
> //问题描述
> 调用 sp.reset(sp2.get()) 后：
> 
> sp.reset(...) 会先释放原来 sp 所管理的对象，即调用 delete 释放第一个 Point 对象。
> 
> 然后，sp 重新管理 sp2.get() 所指向的对象，即让 sp 指向第二个 Point 对象。
> 
> 结果：此时，第二个 Point 对象同时被 sp 和 sp2 管理（两个智能指针指向同一个对象），而该对象最初由 sp2 管理时的引用计数并未正确调整；之后，当这两个智能指针析构时，就会发生双重删除问题。
> //正确用法：
> 
> 如果想让两个 shared_ptr 管理同一对象，应该直接赋值共享所有权，而不是通过 reset(sp2.get())。例如：
> 
> shared_ptr<Point> sp(new Point(1,2));
> shared_ptr<Point> sp2(new Point(1,2));
> sp = sp2; // 这样 sp 和 sp2 共享同一个 Point 对象
> ```
>
> 
