...
C++ 允许通过 **作用域限定符** 或 **虚继承** 显式指定覆盖的基类虚函数。

#### 方式 1：通过作用域限定符覆盖

```c++
Cppclass Derived : public Base1, public Base2, public Base3 {
public:
    void f() override {
        Base1::f(); // 显式调用 Base1::f() 的实现（非必须）
    }
    // 未覆盖 Base2::f() 和 Base3::f()
};
```

- **覆盖 `Base1::f()`**：`Derived::f()` 默认覆盖第一个基类 `Base1` 的 `f()`，无需显式声明。

- **其他基类未覆盖**：`Base2` 和 `Base3` 的虚表仍指向自己的 `f()`。

  方式 2：通过虚继承统一覆盖（解决菱形继承）

  ```c++
  Cppclass A { virtual void f(); };
  class B : virtual public A { void f() override; };
  class C : virtual public A { void f() override; };
  class D : public B, public C {
  public:
      void f() override; // 覆盖所有虚基类 A 的 f()
  };
  ```

  - 虚继承 (`virtual public A`) 确保 `A` 在派生类中只有一个实例，因此 `D::f()` 会覆盖所有路径中的 `A::f()`。
...
1. 主基类虚函数表（继承列表中第一个基类）：

    

   派生类重写的虚函数会直接替换基类虚表中的对应条目，存放真实函数地址
...
2. 其他基类虚函数表：
   ...
   若派生类重写了某个基类的虚函数，该基类虚表中对应的条目可能存放

   跳转指令（thunk）**而非真实地址。这是因为不同基类可能位于派生类对象的不同偏移位置，需要通过跳转调整**this指针后才能正确调用派生类函数
...
```c++
Cppclass Base1 { virtual void f(); };
class Base2 { virtual void f(); };
class Derived : public Base1, public Base2 {
public:
    void f() override; // 重写两个基类的f()
};
```

- **Base1虚表**：直接存储`Derived::f()`的地址。
- **Base2虚表**：存储一个跳转指令，该指令将`this`指针调整到`Derived`对象的起始位置，再跳转到`Derived::f()`的实际地址
...
void test0(){
    C c;
c.a(); // 调用 C::a()，输出：C::a()
c.b(); // b() 未在 C 中重写，A::b() 和 B::b() 都存在，产生二义性（编译错误或“冲突”）
c.c(); // C::c() 隐藏了 A::c()（覆盖 A::c()），也隐藏了 B::c()，输出：C::c()
c.d(); // 调用 C::d()，输出：C::d()
    
    cout << endl;
    A* pa = &c;
   pa->a(); // 虚函数调用，根据 A 部分的 vtable，实际调用 C::a()，输出：C::a()
   pa->b(); // 通过 A*，只查找 A 部分的 b()（未被 C 重写），调用 A::b()，输出：                  A::b()
   pa->c(); // 虚函数调用，查找 A::c()，C::c() 重写了 A::c()，输出：C::c()
// pa->d(); // 编译错误，因为 A 类中没有 d() 函数
...
注意：关于虚函数 b() 的情况，A 和 B 都定义了虚函数 b()，但 C 没有显式重写 b()，这会导致通过 C 对象调用 b() 时存在二义性问题，因为编译器在 C 中无法决定调用 A::b() 还是 B::b()。
...
虚拟继承的实现通常会使得虚拟基类在派生类中仅保存一份数据，而派生类的每个直接基类（B、C）中只保留指向共享基类（A）的指针。
 这种机制会增加一定的内存开销（存储指针），并引入额外的间接访问，但有效地解决了菱形继承中的数据重复问题和二义性问题。
...
![image-20250403114254972](C:\Users\covr\AppData\Roaming\Typora\typora-user-images\image-20250403114254972.png)



![image-20250403114331802](C:\Users\covr\AppData\Roaming\Typora\typora-user-images\image-20250403114331802.png)



![image-20250403114344744](C:\Users\covr\AppData\Roaming\Typora\typora-user-images\image-20250403114344744.png)



![image-20250403114417092](C:\Users\covr\AppData\Roaming\Typora\typora-user-images\image-20250403114417092.png)



**性能影响示例**：

- **虚函数调用**：单一继承的虚函数调用比非虚函数慢约 **10%~20%**（因间接跳转和缓存不命中）。
- **虚拟继承**：访问虚基类成员的耗时可能比普通成员多 **2~3倍**（因多级指针解引用）。

### **实际编程建议**

1. **优先使用单一继承**：结构简单、效率高，避免复杂内存布局问题。
2. **慎用多重继承**：仅在需要组合多个接口时使用，注意避免菱形继承。
3. **虚拟继承仅用于解决菱形问题**：如必须共享基类子对象（如`iostream`继承`istream`和`ostream`）。
4. **避免频繁访问虚基类成员**：将高频访问的数据放在非虚基类中。