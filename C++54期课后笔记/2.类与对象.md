...
这主要与 C++ 的**编译链接机制**以及**单一定义规则（ODR, One Definition Rule）**有关。

1. **预处理展开**：当你使用 `#include` 包含一个头文件时，预处理器会将头文件的所有内容原封不动地复制到当前的源文件（`.cpp`）中。
2. **独立编译**：C++ 中每个源文件是独立编译的。如果在头文件中定义了一个函数（写了完整的函数体），那么每个包含该头文件的 `.cpp` 文件都会生成一份该函数的独立机器码，并保存在各自的目标文件（`.obj` 或 `.o`）中。
3. **链接报错**：在最后的链接阶段，链接器（Linker）尝试将所有的目标文件合并成一个可执行程序。当它发现同一个函数符号在多个目标文件中都有定义时，就会报出**“符号重定义（Redefinition Error）”**的错误。

对于类的成员函数，也存在同样的问题：

**如果在头文件中采用成员函数声明和定义分离的形式，但在类外部（依然在头文件中）完成了成员函数的实现，一旦多个源文件包含此头文件，就会陷入重定义错误。**

**如何解决这个问题呢？我们有三种常用的方案：**

**解决方法 1：将成员函数声明和定义完全分离（最常用、最推荐的标准做法）**
<span style=color:red;background:yellow>**函数声明放在头文件（`.h`），函数定义放在实现文件中（`.cpp`）**</span>。
这是工业界最标准的做法。即使有多个源文件包含头文件，它们也只得到了函数的声明；函数的真实定义只在它对应的那个 `.cpp` 编译单元中出现一次，因此不仅不会出现重定义，还能大幅提高项目的增量编译速度。

**解决方法 2：在成员函数的定义前加上 `inline` 关键字**
`inline`（内联）关键字会告诉编译器及链接器，这个函数可以在多处存在定义，链接器在链接时会自动保留其中一份而丢弃多余的副本。因此，`inline` 函数定义在头文件中是安全的。

<img src="https://bray07.oss-cn-beijing.aliyuncs.com/undefined202403071132784.png" alt="image-20240307113202693" style="zoom:67%;" />

**解决方法 3：将成员函数直接放到类内部进行定义**
在 C++ 中，直接写在 `class` 或 `struct` 内部的成员函数（包括函数体），编译器会**默认将其视为 `inline` 函数**（<font color=red>**即隐式内联**</font>）。因此它也不会导致重定义错误。这种方式特别适合简短的（如 1~3 行）存取器函数。
...
##### 1. 内存对齐基本规则
* **自身对齐值**：每个数据成员在内存中的起始偏移量（Offset），必须是其自身基本类型大小与编译器对齐参数（通常为 8）中较小者的倍数。
  * 对于数组 `char _gArr[20]`：计算对齐时是按其基本元素类型 `char`（1 字节）来决定对齐地址，而不是按整个数组的 20 字节。其总大小为 20 字节。
  * `int` 类型：自身大小为 4 字节，起始偏移量必须是 4 的倍数。
  * `double` 类型：自身大小为 8 字节，起始偏移量必须是 8 的倍数。
* **整体对齐值**：类的总大小必须是类中最大基本类型成员大小（数组拆开按元素算，这里最大的是 `double` 占 8 字节）的整数倍，如果不足则在尾部填充字节。

##### 2. 详细分配过程（从偏移量 0 开始计算）

| 成员变量 | 类型与大小 | 自身对齐要求 | 实际分配的偏移量区间 | 占用字节数 | 解释说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `_gArr` | `char[20]` (20 字节) | 1 字节对齐 | `[0, 19]` | 20 字节 | 起始偏移量为 0（能整除 1），分配后下一个可用偏移量为 20。 |
| `_g1` | `int` (4 字节) | 4 字节对齐 | `[20, 23]` | 4 字节 | 当前偏移量为 20。因为 `20 % 4 == 0`，满足对齐要求，无需填充。分配后下一个可用偏移量为 24。 |
| `_g2` | `double` (8 字节) | 8 字节对齐 | `[24, 31]` | 8 字节 | 当前偏移量为 24。因为 `24 % 8 == 0`，满足对齐要求，无需填充。分配后下一个可用偏移量为 32。 |

##### 3. 整体对齐收尾
* 目前所有成员占用的空间范围是 `[0, 31]`，共 **32 字节**。
* 类的最大基本数据成员是 `_g2` (`double`)，因此该类的整体对齐值是 **8 字节**。
* 类的总大小必须是 8 的整数倍。当前的已占用空间 32 字节刚好是 8 的倍数 (`32 % 8 == 0`)。
* 因此，不需要在尾部填充任何额外的空白字节，最终 `sizeof(G) = 32`。
...
在 C++ 中，根据对象定义的位置和修饰符的不同，对象可以分为**全局对象**、**局部栈对象**、**静态局部对象**和**堆对象**。它们在存储区域、生命周期以及构造/析构调用时机上有着重要的区别：

#### 1. 核心对比表

| 对象类型 | 存储区域 | 构造函数调用时机 | 析构函数调用时机 | 内存管理方式 | 作用域 (可见性) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **全局对象** | 全局/静态区 | `main` 函数执行之前 | 整个程序结束时 | 系统自动分配与回收 | 整个项目/文件内可见 |
| **局部对象 (栈对象)** | 栈区 (Stack) | 代码运行到该对象定义处 | 离开其所在的作用域 (大括号) | 系统自动分配与回收 | 仅在定义它的大括号 `{}` 内可见 |
| **静态局部对象** | 全局/静态区 | **第一次**运行到该对象定义处 | 整个程序结束时 | 系统自动分配与回收 | 仅在定义它的函数/大括号内可见 |
| **堆对象** | 堆区 (Heap) | 执行 `new` 表达式时 | 执行 `delete` 表达式时 | **程序员手动**控制 | 通过指针传递，只要有指针即可访问 |

#### 2. 详细特性与示例代码

为了具体演示它们的调用时机，我们设计以下测试类：
...
public:
    Test(const char* name) : _name(name) {
        std::cout << _name << " : 构造函数被调用" << std::endl;
    }
    ~Test() {
        std::cout << _name << " : 析构函数被调用" << std::endl;
    }
private:
    const char* _name;
};
...
##### A. 全局定义的对象
* **定义位置**：在所有函数外部定义。
* **时机**：每当程序开始运行，在主函数 `main` 接受程序控制权之前，就调用构造函数创建全局对象。当整个程序结束（`main` 执行完毕或调用 `exit`）时，自动调用全局对象的析构函数。
* **代码演示**：
  ```cpp
  Test g_obj("全局对象 g_obj"); // 在 main 之前构造
...
##### B. 局部定义的对象 (栈对象)
* **定义位置**：在函数或普通大括号 `{}` 块内部定义。
* **时机**：每当程序流程到达该对象的定义处就调用构造函数。在程序离开局部对象的作用域（即大括号 `{}` 结束）时，系统自动调用其析构函数，释放栈空间。
* **代码演示**：
  ```cpp
  int main() {
      std::cout << "--- 进入 main 函数 ---" << std::endl;
      {
          Test local_obj("局部栈对象 local_obj");
      } // 离开局部作用域，自动在此处析构
      std::cout << "--- 退出 main 函数 ---" << std::endl;
      return 0;
  }
  // 输出结果：
  // --- 进入 main 函数 ---
  // 局部栈对象 local_obj : 构造函数被调用
  // 局部栈对象 local_obj : 析构函数被调用
  // --- 退出 main 函数 ---
  ```

##### C. 关键字 `static` 定义的静态局部对象
* **定义位置**：在函数内部使用 `static` 修饰定义。
* **时机**：当程序流程**第一次**到达该静态对象定义处时调用构造函数。之后多次调用该函数时，该对象**不再重新构造**（只初始化一次）。在整个程序结束（如 `main` 执行完毕）时，自动调用析构函数。
* **代码演示**：
  ```cpp
  void runDemo() {
      std::cout << "进入 runDemo" << std::endl;
      static Test static_obj("静态局部对象 static_obj");
      std::cout << "离开 runDemo" << std::endl;
  }

  int main() {
      std::cout << "--- 第一次调用 runDemo ---" << std::endl;
      runDemo();
      std::cout << "--- 第二次调用 runDemo ---" << std::endl;
      runDemo(); // 静态对象已存在，不再构造
      std::cout << "--- 退出 main 函数 ---" << std::endl;
      return 0;
  }
  // 输出结果：
  // --- 第一次调用 runDemo ---
  // 进入 runDemo
  // 静态局部对象 static_obj : 构造函数被调用
  // 离开 runDemo
  // --- 第二次调用 runDemo ---
  // 进入 runDemo
  // 离开 runDemo
  // --- 退出 main 函数 ---
  // 静态局部对象 static_obj : 析构函数被调用
  ```

##### D. 用 `new` 运算符创建的堆对象
* **定义位置**：使用 `new` 动态申请。
* **时机**：在程序执行到 `new ClassName(...)` 语句时即刻调用构造函数。在程序员显式使用 `delete` 释放该对象时，才调用析构函数。
* **警告**：如果忘记编写 `delete` 语句，该堆对象的析构函数**永远不会被调用**，会造成严重的内存泄漏。
* **代码演示**：
  ```cpp
  int main() {
      std::cout << "--- 准备申请堆内存 ---" << std::endl;
      Test* p_heap = new Test("堆对象 heap_obj"); // 此时调用构造函数

      std::cout << "--- 准备释放堆内存 ---" << std::endl;
      delete p_heap; // 必须手动释放，此时调用析构函数
  ...
  // 输出结果：
  // --- 准备申请堆内存 ---
  // 堆对象 heap_obj : 构造函数被调用
  // --- 准备释放堆内存 ---
  // 堆对象 heap_obj : 析构函数被调用
  // --- 退出 main 函数 ---
  ```
...
```cpp
Point pt1(1, 2);
Point pt2(pt1);      // 显式调用拷贝构造函数
Point pt3 = pt1;     // 隐式调用拷贝构造函数（注：这里是初始化，不是赋值）
```
* **注意**：如果两个对象都已经存在，再进行等号操作（例如 `pt3 = pt2`），调用的是**赋值运算符函数**，而不是拷贝构造函数。

##### 2. 当函数参数为对象（值传递），形参与实参结合时（实参初始化形参）

当形参是普通对象时，调用函数会产生一个临时的拷贝副本。
```cpp
void showPoint(Point p) { // 形参是普通对象
    p.print();
}

void test() {
    Point pt(10, 20);
    showPoint(pt); // 实参 pt 传递给形参 p，此处隐式调用拷贝构造函数
}
```
   —— **优化建议**：为了避免这次不必要的拷贝开销，建议**使用 `const` 引用作为参数**：
```cpp
void showPoint(const Point & p) { // 不会发生拷贝，仅起别名作用
    p.print();
}
```
...
当函数采用**值返回**方式时，在返回时会产生一个临时对象。
```cpp
Point getPoint() {
    Point temp(1, 2);
    return temp; // 执行 return 时，会调用拷贝构造函数生成临时对象
}
```
   —— **优化建议**：为了避免这次多余的拷贝，可以使用**引用作为返回值**，但**一定要确保返回对象的生命周期大于函数的生命周期**（千万不能返回局部栈对象的引用，但可以返回静态对象或堆对象的引用）。
```cpp
// ❌ 错误做法：temp 在函数结束时被销毁，返回的引用会指向无效的栈空间
Point& getPointError() {
    Point temp(1, 2);
    return temp; 
}

// ✅ 正确做法：静态对象生命周期贯穿程序，可以安全返回引用
Point& getPointSuccess() {
    static Point temp(1, 2);
    return temp; 
}
```
...
**关于返回值优化的说明**：
在现代编译器中，默认会启用 **RVO / NRVO（返回值优化）**。因此在直接编译时可能观察不到拷贝构造函数的调用。
如果要关闭这一优化以观察拷贝构造的调用过程，可以在编译时加上 `-fno-elide-constructors` 参数：
```bash
...
—— **类名(类名 &)** 形式在语法上编译器本身不会报错，但它有两个致命缺陷：

#### 1. 第一个用意：确保右操作数（源对象）的数据成员不被改变（安全保护）
拷贝构造函数的本意是利用已存在的“原件”复制出“新件”，不应该破坏原件。
* 如果**去掉了 `const`**，在拷贝构造函数内部就允许出现修改原件的操作而不报错：
  ```cpp
  Computer(Computer & rhs) {
      _price = rhs._price; 
      rhs._price = 0.0; // ⚠️ 允许此操作，但逻辑上是错误的，原件被破坏
  }
  ```
* 如果**加上了 `const`**，一旦尝试修改 `rhs`，编译器就会直接报错拦截，保障了只读安全性。

#### 2. 第二个用意：为了能够复制临时对象的内容（左值与右值绑定规则限制）
非 `const` 引用不能绑定临时变量（右值）。

##### A. 概念补充：左值与右值
* **左值（Lvalue）**：有确定内存地址、可以取地址、生命周期较长的变量。例如：`int num = 1;` 中的 `num`，我们可以使用 `&num` 取其地址。
* **右值（Rvalue）**：通常是临时的、无名的、不能取地址的数据（临时对象、匿名对象、常量字面量）。例如：数字字面量 `1`、临时匿名对象 `Computer("apple", 12000)`。

##### B. C++ 引用的绑定规则
* **非常量左值引用（`Type &`）**：只能绑定到非常量左值，**绝对不能绑定到右值**。
  ```cpp
  Computer & rhs = Computer("apple", 12000); // ❌ 编译报错！右侧是临时匿名对象（右值）
  ```
* **常量左值引用（`const Type &`）**：是万能引用，**既可以绑定左值，也可以绑定右值**。
  ```cpp
  const Computer & rhs = Computer("apple", 12000); // ✅ 编译通过！
  ```

##### C. 为什么不加 `const` 会在拷贝时报错？
如果在代码中写入：
```cpp
Computer pc3 = Computer("ASUS", 5000); // 右侧是临时对象（右值）
```
若拷贝构造函数为非常量引用 `Computer(Computer & rhs)`，根据上述绑定规则，形参 `rhs` 无法绑定到右侧的临时匿名对象上，编译器就会直接报错：
```
CopyComputer.cc:61:20: error: cannot bind non-const lvalue reference of type 'Computer&' to an rvalue of type 'Computer'
```

<img src="https://bray07.oss-cn-beijing.aliyuncs.com/undefined202403071745528.png" alt="image-20240307174524440" style="zoom:67%;" />

**总结**：`const` 引用可以绑定右值（临时对象），而非 `const` 引用不能绑定右值。
